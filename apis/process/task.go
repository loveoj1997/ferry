...
// 任务列表
func TaskListOld(c *gin.Context) {
	var (
		err        error
		pageValue  pagination.ListRequest
		taskName   string
...
// 创建任务
func CreateTaskOld(c *gin.Context) {
	var (
		err       error
		taskValue struct {
			Name     string `json:"name"`
...
// 更新任务
func UpdateTaskOld(c *gin.Context) {
	var (
		err  error
		file struct {
			Name     string `json:"name"`
...
// 删除任务
func DeleteTaskOld(c *gin.Context) {
	fullName := c.DefaultQuery("full_name", "")
	if fullName == "" || strings.Contains(fullName, "/") {
		app.Error(c, -1, errors.New("参数不正确，请确定参数full_name是否传递"), "")
		return
...
// 任务详情
func TaskDetailsOld(c *gin.Context) {
	var (
		err      error
		fileName string
		content  []byte
